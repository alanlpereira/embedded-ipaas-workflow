...
export const LegalDashboardPage: React.FC<LegalDashboardPageProps> = ({
  ...
  const [movements, setMovements] = useState<ProcessMovement[]>([
    {
      ...
      movement_text: 'Despacho/Intimação: Compulsando os autos, verifica-se que a petição inicial preenche os requisitos do art. 319 do CPC. CITE-SE e INTIME-SE a parte ré EBL Logística S.A. para apresentar contestação no prazo legal de 15 (quinze) dias úteis (art. 335, CPC), sob pena de revelia.',
    ...
    },
    {
      ...
      movement_text: 'Decisão Interlocutória: Tendo em vista que a conciliação restou infrutífera, intimem-se os litigantes para que, no prazo comum de 5 (cinco) dias úteis, especifiquem justificadamente as provas que pretendem produzir na instrução processual.',
    ...
    },
    {
      ...
      movement_text: 'Intimação Eletrônica: Fica o advogado Dr. Alan Pereira (OAB/MG 145105) intimado da juntada de contestação e documentos pelo Estado de Minas Gerais, para que apresente Impugnação/Réplica no prazo legal de 15 (quinze) dias úteis.',
    ...
    },
    {
      ...
      movement_text: 'Notificação PJe: Ficam as partes intimadas da juntada do laudo pericial técnico referente às condições de trabalho. Prazo sucessivo de 10 (dez) dias úteis para manifestação sobre as conclusões do perito.',
    ...
    },
    {
      ...
      movement_text: 'Despacho/Decisão: Detalhamento de ordem judicial de bloqueio via SISBAJUD juntado aos autos com indisponibilidade dos ativos financeiros. Fica a parte executada intimada para ciência da penhora e interposição de Embargos à Execução.',
  ...
  const handleRunCustomQuery = async () => {
    ...
    showToast(`⚡ Consulta avulsa disparada no PJe Comunica (${startDate} até ${endDate})!`);

    if (onRunNow) {
      await onRunNow();
    }

    setTimeout(() => setIsExecutingQuery(false), 2000);
  };
